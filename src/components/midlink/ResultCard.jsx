...
const ResultCard = forwardRef(({ data, onImageUpdate, onTitleUpdate, t }, ref) => {
    ...
    return (
        ...
        >
            ...
            {/* Content Body - 50% of card total */}
            {/* Using flex-1 and gap to ensure perfect distribution without overflow math errors */}
            <div className="h-[50%] px-[15px] py-[15px] flex flex-col bg-white overflow-hidden gap-[8px]">
                ...
                >
                    <p className="text-slate-700 text-sm sm:text-[14px] leading-relaxed font-semibold line-clamp-5 text-justify">
                        {summary}
                    </p>
                </motion.div>

                {/* Key Points Section - Flexible Height */}
                <motion.div 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ delay: 0.5 }}
                    className="flex-1 w-full flex flex-col min-h-0 overflow-hidden text-xs sm:text-sm text-slate-600"
                >
                ...
                <div className="w-full flex justify-between items-end pt-2 text-xs shrink-0 border-t border-slate-100 mt-auto">
